...
for (const livroInfo of estante2) {
  const livroCard = criarCardLivro(livroInfo);
  document.getElementById("estante2").appendChild(livroCard);
}
...
function removerDaEstante(livroInfo) {
    ...
    const livroParaRemover = estante2.find(livro => {
    return (
    ...
    if (livroParaRemover) {
    estante2.splice(estante2.indexOf(livroParaRemover), 1);
